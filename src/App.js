...
const BUILD_DATE_LABEL = "22/09/2026"; // Data fissa della release, non cambia ogni giorno
...
// Dati del segnalante memorizzati sul suo dispositivo (v9.4).
// Servono solo a precompilare il modulo alla segnalazione successiva: restano
// nel browser di chi segnala, non vengono mai letti da altri.
// Allegati: fino a 5 file, 10 MB ciascuno (v9.4 - prima se ne poteva caricare uno solo)
const MAX_FILE = 5;
const MAX_MB = 10;

const CONTATTI_KEY = "cm_contatti";
const STORICO_KEY = "cm_storico";
const CAMPI_MEMORIZZATI = ["condominio", "scala", "interno", "nome", "email", "telefono"];

const leggiContatti = () => {
  try {
    const v = JSON.parse(localStorage.getItem(CONTATTI_KEY) || "");
    return v && typeof v === "object" ? v : {};
  } catch { return {}; }
};
const salvaContatti = (f) => {
  try {
    const out = {};
    CAMPI_MEMORIZZATI.forEach((k) => { if (f[k]) out[k] = f[k]; });
    localStorage.setItem(CONTATTI_KEY, JSON.stringify(out));
  } catch { /* spazio pieno o modalita' privata: si prosegue senza memoria */ }
};

// Storico delle ultime segnalazioni inviate da questo dispositivo, per il controllo
// sui doppioni. Si conservano solo ticket, condominio e momento dell'invio.
const ORE_DOPPIONE = 24;
const leggiStorico = () => {
  try {
    const v = JSON.parse(localStorage.getItem(STORICO_KEY) || "");
    return Array.isArray(v) ? v : [];
  } catch { return []; }
};
const aggiungiAStorico = (condominio, ticket) => {
  try {
    const limite = Date.now() - ORE_DOPPIONE * 3600 * 1000;
    const storico = leggiStorico()
      .filter((v) => v && v.ts > limite)
      .concat([{ condominio, ticket, ts: Date.now() }])
      .slice(-20);
    localStorage.setItem(STORICO_KEY, JSON.stringify(storico));
  } catch { /* come sopra */ }
};
// Confronto tollerante: "Via Roma 12" e "via  roma 12" sono lo stesso stabile
const chiaveCondominio = (v) => String(v ?? "").toLowerCase().replace(/\s+/g, " ").trim();
const segnalazioneRecente = (condominio) => {
  const limite = Date.now() - ORE_DOPPIONE * 3600 * 1000;
  const k = chiaveCondominio(condominio);
  return leggiStorico().find((v) => v && v.ts > limite && chiaveCondominio(v.condominio) === k) || null;
};

// Scurisce un colore esadecimale della percentuale indicata (v9.5)
const scurisci = (hex, q) => {
  const m = /^#?([0-9a-f]{6})$/i.exec(String(hex || ""));
  if (!m) return hex;
  const n = parseInt(m[1], 16);
  const c = [(n >> 16) & 255, (n >> 8) & 255, n & 255]
    .map((v) => Math.max(0, Math.round(v * (1 - q))));
...
export default function CondoMessenger() {
  ...
  const [primary] = useState(load("cm_primary", "#8B1538"));
  ...
  // Form - i dati anagrafici sono precompilati con quelli dell'ultima segnalazione
  // inviata da questo dispositivo (v9.4).
  ...
  // Segnalazione gia' inviata di recente per lo stesso condominio: si chiede conferma
  ...
  // v9.5 - identita' visiva allineata alle altre webapp dello studio: bordeaux #8B1538.
  // La tonalita' scura viene calcolata dal colore di marca, cosi' resta coerente
  // anche se un domani il colore venisse cambiato dalla configurazione.
  ...
  const validate = (f) => {
    ...
    const files = f.files || [];
    if (f.categoria === "Invio Documenti" && !files.length) errs.push("Carica un file");
  ...
  const submit = async (forzaDoppione = false) => {
    if (cooldown) return;
    ...
    // Controllo doppioni (v9.4): stesso condominio gia' segnalato da questo
    // dispositivo nelle ultime 24 ore. Si chiede conferma, non si blocca.
    if (!forzaDoppione) {
      const prec = segnalazioneRecente(clean.condominio);
    ...
    try {
      ...
      Object.entries(payload).forEach(([k, v]) => { if (k !== "files") fd.append(k, String(v)); });
      // Tutti gli allegati viaggiano sotto la stessa chiave "file1": Make li riceve
      // come elenco e li carica uno per uno nella cartella dell'intervento.
      ...
      setCooldown(8);
      salvaContatti(clean);
      aggiungiAStorico(clean.condominio, ticket);
      setForm((s) => ({ ...s, categoria: "", subcategoria: "", messaggio: "", files: [], consenso: s.consenso }));
      setTouched({});
  ...
  const subCategorieManut = [
    "Antennista", "Ascensore", "Cancelli Elettrici", "Contabilizzazione",
    "Disinfestazioni/Derattizzazioni", "Edilizia", "Elettricista", "Fabbro", "Fognature",
    "Giardinaggio", "Idraulico", "Impianto di Riscaldamento", "Montascale", "Pulizia",
    "Videosorveglianza"
  ];
  ...
  return (
    <div className="relative min-h-screen w-full bg-paper bg-noise text-neutral-900" style={cssVars}>
      ...
      <div aria-hidden="true" className="pointer-events-none fixed inset-0 overflow-hidden z-0">
        ...
        <div className="absolute top-1/2 -right-40 w-96 h-96 rounded-full bg-[var(--brand)]/10 blur-3xl" />
      </div>
      ...
      <AnimatePresence>
        {showInfo && (
          <motion.div
            ...
            className="relative z-10 bg-gradient-to-b from-[var(--brand)]/8 to-[var(--brand)]/4 border-b border-[var(--brand)]/20"
          >
            <div className="max-w-5xl mx-auto px-4 sm:px-6 py-4">
              <div className="flex items-start gap-3">
                <div className="flex-shrink-0 w-9 h-9 rounded-xl bg-[var(--brand)]/12 flex items-center justify-center">
                  <Info className="w-5 h-5 text-[var(--brand-dark)]" />
                </div>
                <div className="flex-1 text-sm">
                  <p className="font-semibold text-[var(--brand-deep)] mb-1">Come funziona</p>
                ...
                <button onClick={() => setShowInfo(false)} className="p-1.5 hover:bg-[var(--brand)]/12 rounded-lg transition-colors">
                  <X className="w-4 h-4 text-[var(--brand-dark)]" />
                </button>
              </div>
            </div>
          </motion.div>
      ...
      <main className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        >
          {/* Card header */}
          <div className="relative overflow-hidden bg-gradient-to-br from-[var(--brand)] via-[var(--brand-dark)] to-[var(--brand-deep)] px-6 sm:px-8 py-6">
              ...
              <CategoryGrid
                ...
                onSelect={(v) => {
                  ...
                  if (v !== "Interventi di Manutenzione" && v !== "Invio Documenti") {
                    setForm(s => ({ ...s, subcategoria: "", files: [] }));
                  } else {
                  ...
                  >
                    ...
                    <div className="mt-4">
                      <FileUpload
                        ...
                        required={false}
                        hint={`Facoltativo, ma molto utile: le foto aiutano il tecnico a capire il problema prima del sopralluogo. Puoi allegarne fino a ${MAX_FILE}.`}
                      />
                    </div>
                  </motion.div>
                )}
              ...
              <motion.button
                ...
                className={cn(
                  ...
                  sending || cooldown
                    ...
                    : "bg-gradient-to-r from-[var(--brand)] via-[var(--brand-dark)] to-[var(--brand-deep)] shadow-lg hover:shadow-xl"
                )}
              >
                {!(sending || cooldown) && (
                  <span
            ...
            <AnimatePresence>
              {doppione && (
                <motion.div
                  ...
                  className="rounded-2xl border border-amber-300 bg-gradient-to-br from-amber-50 to-white p-5 shadow-soft"
                >
                  <div className="flex items-start gap-4">
                    <div className="flex-shrink-0 w-11 h-11 rounded-2xl bg-amber-100 flex items-center justify-center">
                      <AlertCircle className="w-6 h-6 text-amber-700" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="font-display font-semibold text-amber-900 text-lg leading-tight">
                        Hai già segnalato questo condominio
                      </div>
                      <p className="text-sm text-amber-800/90 mt-1">
                        Nelle ultime 24 ore da questo dispositivo è partita una segnalazione per
                        <strong> {form.condominio}</strong>, con ticket{" "}
                        <span className="font-mono font-semibold">{doppione.ticket}</span>.
                        Se si tratta di un problema diverso puoi procedere, altrimenti è meglio
                        non aprire un secondo ticket per la stessa cosa.
                      </p>
                      <div className="mt-3 flex flex-col sm:flex-row gap-2">
                        <button
                          type="button"
                          onClick={() => submit(true)}
                          className="px-4 py-2 rounded-xl bg-amber-600 text-white text-sm font-semibold hover:bg-amber-700 transition-colors"
                        >
                          È un problema diverso, invia
                        </button>
                        <button
                          type="button"
                          onClick={() => setDoppione(null)}
                          className="px-4 py-2 rounded-xl bg-white ring-1 ring-neutral-300 text-neutral-700 text-sm font-semibold hover:bg-neutral-50 transition-colors"
                        >
                          Annulla
                        </button>
                      </div>
              ...
              {result && (
                <motion.div
                  ...
                  className={cn(
                    ...
                    result.ok
                      ? "bg-gradient-to-br from-[var(--brand)]/8 to-white border-[var(--brand)]/30"
                ...
                >
                  <div className="flex items-start gap-4">
                    <div className={cn(
                      ...
                      result.ok ? "bg-[var(--brand)]/12" : "bg-red-100"
                    )}>
                      {result.ok ? (
                        <CheckCircle2 className="w-6 h-6 text-[var(--brand-dark)]" />
                      ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      {result.ok ? (
                        <>
                          <div className="font-display font-semibold text-[var(--brand-deep)] text-lg leading-tight">Segnalazione inviata!</div>
                          <p className="text-sm text-neutral-600 mt-1">La tua richiesta è stata registrata correttamente.</p>
                          {result.ticket && (
                            <div className="mt-3 bg-white rounded-xl px-4 py-3 ring-1 ring-[var(--brand)]/20">
                              <p className="text-[11px] uppercase tracking-wider text-neutral-500 mb-1 font-semibold">Numero ticket</p>
                              <p className="font-mono font-bold text-[var(--brand-dark)] text-lg tracking-tight select-all break-all">{result.ticket}</p>
                            </div>
                          )}
                        </>
                      ) : (
                    ...
                    <button
                      ...
                      aria-label="Chiudi"
                    >
                      <X className={cn("w-4 h-4", result.ok ? "text-[var(--brand-dark)]" : "text-red-700")} />
                    </button>
                  </div>
                </motion.div>
              )}
            ...
            <div className="flex items-start gap-3">
              <div className={cn(
                ...
                isSelected
                  ...
                  // v9.4 - la card principale aveva un fondo verde chiarissimo che sul suo
                  // sfondo, anch'esso verde chiaro, spariva: ora usa lo stesso riquadro
                  // grigio delle altre, con l'icona nel colore del brand.
                  : isPrimary
              ...
              <div className="flex-1 min-w-0 pr-5">
                <div className={cn(
                  "font-semibold text-base mb-1 leading-tight min-h-[2.75rem]",
                  isSelected || isPrimary ? "text-[var(--brand)]" : "text-neutral-900"
                )}>
                  {cat.value}
                  {isPrimary && (
                    <span className="ml-2 inline-flex items-center px-1.5 py-0.5 rounded-md text-[10px] font-bold bg-[var(--brand)] text-white tracking-wider">
                      PRINCIPALE
                    </span>
                  )}
                </div>
...
function FileUpload({ files = [], onChange, label = "Carica file", hint, required = true }) {
  const inputRef = React.useRef(null);
  const [dragOver, setDragOver] = useState(false);
  const pieno = files.length >= MAX_FILE;

  // I nuovi file si aggiungono a quelli gia' scelti, saltando i doppioni per
  // nome e dimensione e fermandosi al limite.
    ...
    <div>
        ...
        <div className="space-y-2 mb-3">
          {files.map((f, i) => (
            <motion.div
              ...
              className="flex items-center gap-3 p-3 bg-[var(--brand)]/5 border border-[var(--brand)]/25 rounded-2xl"
            >
              <div className="w-9 h-9 rounded-xl bg-[var(--brand)]/15 flex items-center justify-center flex-shrink-0">
                <Paperclip className="w-4 h-4 text-[var(--brand)]" />
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-neutral-900 truncate">{f.name}</p>
                <p className="text-xs text-neutral-600">{(f.size / 1024).toFixed(0)} KB</p>
              </div>
              <button
                type="button"
      ...
      {!pieno && (
        <label
          ...
          onDrop={handleDrop}
          className={cn(
            "flex flex-col items-center justify-center w-full border-2 border-dashed rounded-2xl transition-all cursor-pointer",
            files.length ? "h-24" : "h-36",
            dragOver
        ...
        >
          <div className="flex flex-col items-center justify-center px-6 text-center">
            ...
            <p className="text-sm text-neutral-700 font-semibold">
              {files.length ? "Aggiungi un altro file" : "Clicca o trascina i file qui"}
            </p>
            <p className="text-xs text-neutral-500 mt-1">
              PDF, JPG, JPEG o PNG · max {MAX_MB} MB ciascuno · fino a {MAX_FILE} file
            </p>
          </div>
          <input
            ref={inputRef}
      ...
      {pieno && (
        <p className="text-xs text-neutral-500">
          Hai raggiunto il limite di {MAX_FILE} file. Rimuovine uno per aggiungerne un altro.
        </p>
      )}
    </div>
  );
