...
import { Send, CheckCircle2, AlertCircle, Building2, Mail, Phone, Shield, Loader2, ChevronDown } from "lucide-react";
...
export default function CondoMessenger() {
  ...
  const validate = () => {
    ...
    if (form.categoria === "Manutenzione" && !form.subcategoria) errs.push("Seleziona una sottocategoria di manutenzione");
  ...
  const submit = async () => {
    ...
    try {
      ...
      setCooldown(8);
      setForm((s) => ({ ...s, categoria: "", subcategoria: "", messaggio: "", consenso: s.consenso }));
    } catch (e) {
  ...
  const categorie = ["Amministrativa","Contabile","Documentazione","Manutenzione","Altro"];
