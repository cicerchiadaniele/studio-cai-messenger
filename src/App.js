...
export default function CondoMessenger() {
  ...
  const validate = () => {
    ...
    if (form.categoria === "Invio Documenti") {
      if (form.files.length === 0) errs.push("Carica almeno un file");
      if (form.files.length > 3) errs.push("Puoi caricare al massimo 3 file");
      const invalid = form.files.find(f => !/\.(pdf|jpg|jpeg|png)$/i.test(f.name));
      if (invalid) errs.push("Sono ammessi solo PDF o immagini (jpg, jpeg, png)");
    }
    return errs;
  };
  ...
  const submit = async () => {
    ...
    try {
      ...
      // Attach files as file1..file3 with filename for Make/Gmail
      form.files.slice(0,3).forEach((file, idx) => {
        fd.append(`file${idx+1}`, file, file.name);
      });
  ...
  const subCategorieManut = ["Elettricista","Antennista","Fabbro","Idraulico","Edilizia","Cancelli Elettrici","Disinfestazioni/Derattizzazioni"];
    ...
    <div className="min-h-screen w-full bg-neutral-50 text-neutral-900" style={cssVars}>
      ...
      <main className="max-w-4xl mx-auto px-4 py-6">
        <motion.div layout className="bg-white rounded-2xl shadow p-4">
          ...
          {form.categoria === "Invio Documenti" && (
            <div className="mt-3">
              <label className="text-sm font-medium flex items-center gap-2"><Paperclip className="w-4 h-4"/>Carica fino a 3 file (PDF o immagini)</label>
              <input
                type="file"
                accept=".pdf,.jpg,.jpeg,.png"
                multiple
                onChange={(e)=>update("files", Array.from(e.target.files).slice(0,3))}
              ...
              />
              {form.files.length > 0 && (
                <ul className="mt-2 text-xs text-neutral-600 list-disc pl-5">
